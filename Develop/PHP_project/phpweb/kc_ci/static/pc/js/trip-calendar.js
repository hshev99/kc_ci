...
Y.TripCalendar = Y.extend(Calendar, Y.Base, {
    ...
    _setWidth: function() {
        (function(that, boundingBox, contentBox) {
            boundingBox.all('.inner, h4').setStyle('width', boundingBox.one('dl').get('offsetWidth'));
    ...
    _setDateStyle: function() {
        var boundingBox   = this.boundingBox,
            ...
            oStartDate    = startDate && boundingBox.one('li[data-date="' + startDate + '"]'),
            oEndDate      = endDate && boundingBox.one('li[data-date="' + endDate + '"]'),
            oSelectedDate = selectedDate && boundingBox.one('li[data-date="' + selectedDate + '"]'),
            iDiff         = this._getDateDiff(startDate, endDate),
            aTd           = boundingBox.all('li'),
            oTd           = null;
        ...
        //oSelectedDate && oSelectedDate.removeClass('selected-date');
        if(oSelectedDate && oSelectedDate.hasClass('selected-date')){
            oSelectedDate && oSelectedDate.removeClass("on");
        }else{
            oSelectedDate && oSelectedDate.addClass("on");
        }
        ...
        for(var i = 0; i < iDiff - 1; i++) {
            ...
            oTd       = boundingBox.one('li[data-date="' + startDate + '"]');
    ...
    _mouseenter: function(oTarget) {
        ...
        boundingBox.all('li').removeClass('hover');
    ...
    _mouseleave: function() {
        (function(that) {
            ...
            that.leaveTimer = setTimeout(function() {
                that.boundingBox.all('li').removeClass('hover');
            }, 30);
        })(this);
    },
...
},
{
    ...
    HEAD_TEMPLATE: '<li class="{weekend}">{weekday_name}</li>',
    
    BODY_TEMPLATE: '<ul>' +
                        '{calday_row}' +
                    '</ul>',
                    
    DAY_TEMPLATE: '<li data-date="{date}" class="{disabled}">' +
