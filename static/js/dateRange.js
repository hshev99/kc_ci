...
*/
    ...
    var defaults = {
        ...
        disCertainDates : {},//对象方式传入下标代表月份，后面代表要灰色的内容
 ...
 */
pickerDateRange.prototype.selectDate = function(ymd) {console.log(ymd);
...
pickerDateRange.prototype.fillDate = function(year, month, index) {
    ...
    for(var d = dateBegin; d.getTime() <= dateEnd.getTime(); d.setDate(d.getDate() + 1)) {
        if(d.getTime() < firstDayOfMonth.getTime()) { // 当前月之前的日期
            ...
            //让那年那月几号不可选
            if(this.mOpts.disCertainDates[d.getFullYear()][d.getMonth()+1] && this.mOpts.disCertainDates[d.getFullYear()][d.getMonth()+1].length > 0 ){
                var isDisabled = false;

                for(var p in this.mOpts.disCertainDates[d.getFullYear()][d.getMonth()+1]){
                    if(!isNaN(this.mOpts.disCertainDates[d.getFullYear()][d.getMonth()+1][p]) || isNaN(parseInt(this.mOpts.disCertainDates[d.getFullYear()][d.getMonth()+1][p]))){
                        if ( this.mOpts.disCertainDates[d.getFullYear()][d.getMonth()+1][0] === true ){
                            isDisabled = !!(d.getDate() !== this.mOpts.disCertainDates[d.getFullYear()][d.getMonth()+1][p]);
                            if ( !isDisabled ){
                                break;
                            }
                        }else {
